...
private:
    ...
    TaskHandle_t urc_task_handle_ = nullptr;    // Task for dispatching URC callbacks asynchronously
    QueueHandle_t rx_data_queue_;  // Queue for DMA received data
    QueueHandle_t urc_queue_ = nullptr;  // Queue for URC dispatch items (heap-allocated)
    ...
    void UrcDispatchTask(); // Task for running URC callbacks off the event loop
    bool ParseResponse();
