...
// URC dispatch item - stores parsed URC for asynchronous callback execution
// Allocated on heap, freed by the dispatch task after running callbacks
struct UrcItem {
    std::string command;
    std::vector<AtArgumentValue> arguments;
};

// Maximum URC items queued before fallback to synchronous dispatch
#define AT_URC_QUEUE_DEPTH 64
...
AtUart::~AtUart() {
    ...
    if (urc_task_handle_) {
        vTaskDelete(urc_task_handle_);
    }
    if (event_group_handle_) {
        vEventGroupDelete(event_group_handle_);
    }
    if (urc_queue_) {
        UrcItem* item = nullptr;
        while (xQueueReceive(urc_queue_, &item, 0) == pdTRUE) {
            delete item;
        }
        vQueueDelete(urc_queue_);
    }
    if (rx_data_queue_) {
...
void AtUart::Initialize() {
    ...
    // EventTask: handles parsing only - dispatches URCs asynchronously
    xTaskCreate([](void* arg) {
        auto at_uart = (AtUart*)arg;
        at_uart->EventTask();
        vTaskDelete(NULL);
    ...
    // UrcDispatchTask: runs user URC callbacks off the EventTask so heavy
    // callbacks (e.g. opus decode + audio enqueue) don't block AT response parsing.
    urc_queue_ = xQueueCreate(AT_URC_QUEUE_DEPTH, sizeof(UrcItem*));
    if (!urc_queue_) {
        ESP_LOGE(TAG, "创建URC队列失败");
        return;
    }
    xTaskCreate([](void* arg) {
        auto at_uart = (AtUart*)arg;
        at_uart->UrcDispatchTask();
        vTaskDelete(NULL);
    }, "modem_urc", 2048 * 3, this, configMAX_PRIORITIES - 4, &urc_task_handle_);
...
void AtUart::HandleUrc(const std::string& command, const std::vector<AtArgumentValue>& arguments) {
    if (command == "CME ERROR") {
        // CME ERROR is the response to the in-flight AT command. Must be
        // signalled synchronously so SendCommand wakes up immediately.
        if (!arguments.empty()) {
    ...
    // Decide which URCs are "heavy" (carry payload that drives slow user code,
    // e.g. opus decode + audio enqueue) and which are "light" control URCs
    // that just flip event-group bits. Light URCs MUST stay inline so things
    // like SendCommand's `+MIPSEND: id,len` confirmation, `+MHTTPURC` header
    // and `disconn` URCs aren't queued behind audio data URCs.
    bool is_heavy = false;
    if (command == "MIPURC" && !arguments.empty()
        && (arguments[0].string_value == "rtcp" || arguments[0].string_value == "rudp")) {
        // TCP / UDP 入站数据：回调里要走 hex 解码 + opus 解码 + 音频入队，比较重。
        is_heavy = true;
    } else if (command == "MHTTPURC" && !arguments.empty()
        && (arguments[0].string_value == "content" || arguments[0].string_value == "recv")) {
        is_heavy = true;
    } else if (command == "MQTTURC" && !arguments.empty()
        && arguments[0].string_value == "publish") {
        // MQTT 下行消息 payload 较大，避免堵 EventTask。
        is_heavy = true;
    }
    ...
    // Heavy URC: hand off to dispatcher so EventTask can keep parsing.
    UrcItem* item = new (std::nothrow) UrcItem{command, arguments};
    if (!item) {
        ESP_LOGW(TAG, "URC item alloc failed, falling back to sync dispatch");
        std::lock_guard<std::mutex> lock(urc_mutex_);
        for (auto& callback : urc_callbacks_) {
            callback(command, arguments);
        }
    ...
    if (xQueueSend(urc_queue_, &item, 0) != pdTRUE) {
        // Queue full: dispatch synchronously to avoid losing the URC. Logs a
        // warning so we know the dispatcher is lagging behind.
        ESP_LOGW(TAG, "URC queue full, dispatching '%s' synchronously", command.c_str());
        delete item;
        std::lock_guard<std::mutex> lock(urc_mutex_);
        for (auto& callback : urc_callbacks_) {
            callback(command, arguments);
...
void AtUart::UrcDispatchTask() {
    while (true) {
        UrcItem* item = nullptr;
        if (xQueueReceive(urc_queue_, &item, portMAX_DELAY) != pdTRUE || !item) {
            continue;
        }
        {
            std::lock_guard<std::mutex> lock(urc_mutex_);
            for (auto& callback : urc_callbacks_) {
                callback(item->command, item->arguments);
            }
        }
        delete item;
    }
