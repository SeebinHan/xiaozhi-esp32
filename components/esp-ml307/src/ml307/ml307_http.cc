...
int Ml307Http::Write(const char* buffer, size_t buffer_size) {
    ...
    while (buffer_sent < buffer_size) {
        ...
        if (!at_uart_->SendCommandWithData(command, 10000, true, buffer + buffer_sent, buffer_chunk_size)) {
            ESP_LOGE(TAG, "MHTTPCONTENT chunk upload failed: chunk=%u sent=%u/%u cme=%d",
                     (unsigned)buffer_chunk_size, (unsigned)buffer_sent, (unsigned)buffer_size,
                     at_uart_->GetCmeErrorCode());
            return buffer_sent;
        }
...
bool Ml307Http::Open(const std::string& method, const std::string& url) {
    ...
    if (protocol_ == "https") {
        // ML307R only accepts these two SSL knobs (version/ignorelocaltime
        // return CME 50). seclevel-equivalent is "auth": 0 = no peer verify.
        at_uart_->SendCommand("AT+MSSLCFG=\"ciphersuite\",0,0xFFFF");
        at_uart_->SendCommand("AT+MSSLCFG=\"auth\",0,0");
    ...
    if (method_supports_content && content_.has_value()) {
        // ML307R 的 MHTTPCONTENT=id,0,N（Content-Length 单次模式）单次最多 ~4KB，
        // 且发完即关 body 流，无法循环再发。Body 大于 4KB 的请求请用 chunked
        // (Transfer-Encoding: chunked + Open + 多次 Write + Write("",0))。
        command = "AT+MHTTPCONTENT=" + std::to_string(http_id_) + ",0," + std::to_string(content_.value().size());
        auto& content = content_.value();
        if (!at_uart_->SendCommandWithData(command, 10000, true, content.data(), content.size())) {
            ESP_LOGE(TAG, "MHTTPCONTENT upload failed: size=%u cme=%d",
                     (unsigned)content.size(), at_uart_->GetCmeErrorCode());
