...
int Ml307Tcp::Send(const std::string& data) {
    ...
    while (total_sent < data.size()) {
        ...
        uint32_t timeout_ms = tx_time_ms + 1000; // 与 Ml307Http/Udp 对齐

        if (!at_uart_->SendCommand(command, timeout_ms, false)) {
            int cme = at_uart_->GetCmeErrorCode();
            ESP_LOGE(TAG, "Failed to send data chunk: chunk=%u total=%u/%u cme=%d",
                     (unsigned)chunk_size, (unsigned)total_sent, (unsigned)data.size(), cme);
            // Only tear down the link when the modem actively rejects (CME ERROR).
            // A pure timeout (cme==0) can be a transient AT-pipe stall under high
            // RX URC load; keep the socket and let the upper layer decide.
            if (cme != 0) {
                Disconnect();
            }
            return -1;
