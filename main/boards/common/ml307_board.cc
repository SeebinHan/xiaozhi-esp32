...
Ml307Board::Ml307Board(gpio_num_t tx_pin, gpio_num_t rx_pin, gpio_num_t dtr_pin, int preferred_baud_rate)
    : tx_pin_(tx_pin), rx_pin_(rx_pin), dtr_pin_(dtr_pin), preferred_baud_rate_(preferred_baud_rate) {
}
...
void Ml307Board::NetworkTask() {
    ...
    while (detect_retries < MODEM_DETECT_MAX_RETRIES) {
        modem_ = AtModem::Detect(tx_pin_, rx_pin_, dtr_pin_, preferred_baud_rate_);
        if (modem_ != nullptr) {
            break;
        }
        detect_retries++;
