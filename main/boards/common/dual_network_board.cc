...
DualNetworkBoard::DualNetworkBoard(gpio_num_t ml307_tx_pin, gpio_num_t ml307_rx_pin, gpio_num_t ml307_dtr_pin, int32_t default_net_type, int ml307_preferred_baud_rate)
    : Board(),
      ml307_tx_pin_(ml307_tx_pin),
      ml307_rx_pin_(ml307_rx_pin),
      ml307_dtr_pin_(ml307_dtr_pin),
      ml307_preferred_baud_rate_(ml307_preferred_baud_rate) {
...
void DualNetworkBoard::InitializeCurrentBoard() {
    if (network_type_ == NetworkType::ML307) {
        ...
        current_board_ = std::make_unique<Ml307Board>(ml307_tx_pin_, ml307_rx_pin_, ml307_dtr_pin_, ml307_preferred_baud_rate_);
    } else {
