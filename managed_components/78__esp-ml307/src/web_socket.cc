...
void WebSocket::OnTcpData(const std::string& data) {
    ...
    while (buffer_offset < buffer_size) {
        ...
        switch (opcode) {
            ...
            case 0x9: // Ping
                if (connected_) {
                    auto pong_payload = payload;
                    std::thread([this, pong_payload = std::move(pong_payload)]() mutable {
                        SendControlFrame(0xA, pong_payload.empty() ? nullptr : pong_payload.data(), pong_payload.size());
                    }).detach();
                }
                break;
            case 0xA: // Pong
